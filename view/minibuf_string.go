...
func MBReadString(prompt, initial string) (string, model.PromptResult) {
	return MBReadStringCap(prompt, initial, model.PatternCapacity)
}
...
func MBReadFilenameString(prompt, initial string) (string, model.PromptResult) {
	p := NewFilenamePrompt(prompt, initial, fileio.PromptPathCapacity)
	p.OpenBlocking()
	defer p.Close()
	for {
		k, ok := WaitKey()
...
func MBReadFuzzyListString(prompt string, provider model.MbNameProviderFn, providerCtx any, providerCount uint) (string, model.PromptResult) {
...
func MBReadFuzzyListExString(prompt string, provider model.MbNameProviderFn, providerCtx any, providerCount uint, displayFormatter model.MbMatchFormatter, displayCtx any) (string, model.PromptResult) {
	buf := make([]byte, model.PatternCapacity)
	if pr, played := TryMacroPlayPrompt(buf); played {
		return promptStringFromBuf(buf), pr
	}
	p := NewFuzzyPrompt(prompt, provider, providerCtx, providerCount, displayFormatter, displayCtx, model.PatternCapacity)
	p.OpenBlocking()
	defer p.Close()
	for {
		k, ok := WaitKey()
...
func AskFuzzy(prompt string, provider model.MbNameProviderFn, providerCtx any, providerCount uint, onDone func(string, model.PromptResult)) {
...
func AskFilename(prompt, initial string, onDone func(string, model.PromptResult)) {
...
func AskChoose(prompt string, ctx any, labelFn model.MLChoiceLabelFn, count uint8, defaultIdx uint8, onDone func(int16)) {
