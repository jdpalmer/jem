...
// minibuf.go - Minibuffer presentation (message line); editing lives in model.
...
func MBHistoryAdd(text string) {
	model.MinibufferHistoryAdd(text)
}
...
func mlFinish(cursorCol int, messagePresent bool) {
	...
	model.State.MessagePresent = messagePresent
}
...
func MBWrite(format string, args ...interface{}) {
	...
	mlBegin(model.State.Theme.NormalStyle)
...
func MBWritePrompt(prompt string, text []byte, cpos int) {
	MBWritePromptStyle(prompt, text, cpos, model.State.Theme.NormalStyle)
}
...
func MBEditKeyHistory(buf []byte, cpos *int, nbuf int, initial []byte, historyPos *int16, haveSavedEdit *bool, savedEdit []byte, k uint32) model.MinibufferEditResult {
	return model.EditKeyHistory(buf, cpos, nbuf, initial, historyPos, haveSavedEdit, savedEdit, k)
}
