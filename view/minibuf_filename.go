...
func completePromptFilename(state *model.MinibufferState) bool {
...
// mbReadFilename prompts for a filename with tab completion and fuzzy matching (blocking).
func mbReadFilename(prompt string, buf []byte, nbuf int) model.PromptResult {
	initial := ""
	if len(buf) > 0 && buf[0] != 0 {
		...
		initial = string(buf[:end])
	}
	p := NewFilenamePrompt(prompt, initial, nbuf)
	p.OpenBlocking()
	defer p.Close()
	for {
		k, ok := WaitKey()
...
func MBReadCommand(buf []byte, nbuf int) model.PromptResult {
