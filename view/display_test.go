...
func TestDisplayUpdateRestoresEditorCursorAfterMessage(t *testing.T) {
	...
	if model.State.Cursor.Row != uint32(term.Rows()) {
		t.Fatalf("MBWrite cursor row = %d, want message row %d", model.State.Cursor.Row, term.Rows())
	}
