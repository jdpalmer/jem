...
func WinAt(r uint32) *model.Window {
...
func MouseLocationInWindow(wp *model.Window) buffer.Location {
	if wp == nil {
		return buffer.Location{}
	}
	rowInWin := model.State.Mouse.Row - wp.ScreenTopRow
	...
	textCol := int(model.State.Mouse.Col) - int(wp.GutterWidth()) + int(wp.HScroll)
	if textCol < 0 {
		textCol = 0
	}
	if line != nil {
