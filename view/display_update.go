...
func renderModeline(wp *model.Window) {
	...
	nameStyle := buffer.MakeTextStyle(model.State.Theme.ModelineNameColor, gutterBg, buffer.TextStyleBold)
	...
	// Spot/mark indicator
	if len(model.MarksState.Marks) > 0 {
	...
	// Editor version
	screenPutBytes([]byte("  jem " + model.Version + " | "))
	...
	if gitText := gitModelineText(bp); gitText != "" {
		gitStyle := buffer.MakeTextStyle(model.State.Theme.ModelineNameColor, gutterBg, 0)
		screenPutBytes([]byte(" | "))
...
func DisplayUpdate() {
	...
	if term.RefreshSize() {
		...
		model.State.MessagePresent = false
	}
	...
	for wi := 0; wi < int(len(model.State.Windows)); wi++ {
		wp := model.State.Windows[wi]
		if wp == nil || wp.Buffer == nil {
			continue
		}
		...
		if wp.DidEdit && !wp.DidMove && !wp.ShouldRedraw {
			...
			if scrollN != 0 && scrollN > -int(wp.Height) && scrollN < int(wp.Height) {
				...
				if absN > 0 && absN < height {
					...
					// Scroll the terminal
					displaySetStyle(model.State.Theme.NormalStyle)
	...
	if model.State.ActiveMinibuffer == nil {
		if model.State.CurrentWindow != nil {
			cw := model.State.CurrentWindow
			cursorLineDelta := cw.Cursor.Line - cw.TopLine
			model.State.Cursor.Row = cw.ScreenTopRow + uint32(cursorLineDelta)
			...
			model.State.Cursor.Col = uint32(cursorCol)
		}
