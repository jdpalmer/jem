...
func allocScreen(rows int) Screen {
	...
	for i := range s.Rows {
		...
		for j := 0; j < cols; j++ {
			s.Rows[i].Text[j] = ' '
			s.Rows[i].Style[j] = model.State.Theme.NormalStyle
		}
	}
	return s
}
...
func displaySetStyle(style buffer.TextStyle) {
	if model.State.ActiveStyle != style {
		term.SetStyle(style)
		model.State.ActiveStyle = style
	}
}
...
func screenMove(row, col int) {
	swCursorRow = row
	swCursorCol = col
	drawStyle = model.State.Theme.NormalStyle
}
...
func screenFlushRow(row, cursorCol int) {
	...
	rowSync(row)
	if row == term.Rows() {
		model.State.Cursor.Row = uint32(term.Rows())
...
func ScreenSync() {
	...
	if model.State.ScreenDirty {
		...
		displaySetStyle(normalStyle)
		term.EraseEos()
		model.State.ScreenDirty = false
		model.State.MessagePresent = false
	}

	// Sync dirty body rows and the message line when active.
	syncRows := term.Rows()
	if model.State.MessagePresent && term.Rows() < len(backScreen.Rows) {
	...
	term.Move(int(model.State.Cursor.Row), int(model.State.Cursor.Col))
	term.Flush()
