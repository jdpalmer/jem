...
func TestOperatorStyleForLang(t *testing.T) {
	...
	if operatorStyleForLang(buffer.LModeGo, "===") != TextStyleDefault {
	...
	if operatorStyleForLang(buffer.LModePython, "++") != TextStyleDefault {
		t.Fatal("expected Python not to treat ++ as an operator")
	}
	if operatorStyleForLang(buffer.LModeNone, "=") != TextStyleDefault {
