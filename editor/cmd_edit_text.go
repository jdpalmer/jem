...
func bufferSetText(bp *buffer.Buffer, begin, end buffer.Location, newText []byte, newEndOut *buffer.Location, kill bool) bool {
	if kill {
		...
		if len(oldText) > 0 && !model.KillAppend(oldText) {
			return false
		}
	}
	err := model.SetText(bp, begin, end, newText, newEndOut)
	if err != nil {
		return false
	}
	if kill {
...
func windowDeleteBytes(wp *model.Window, n int, kill bool) bool {
...
func bufferCharStats(bp *buffer.Buffer, wp *model.Window) (charAt int, before, total uint) {
	if bp == nil || wp == nil {
		return '\n', 0, 0
	}
	cline := uint(1)
...
func CmdShowPosition(f bool, n int) bool {
	...
	col := view.WindowCursorScreenCol(wp)
	ratio := uint(0)
	if total > 0 {
		ratio = (100 * before) / total
	}
	row := int(model.State.Cursor.Row) + 1
	view.MBWrite("X=%d Y=%d CH=0x%x .=%d (%d%% of %d)", col+1, row, charAt, before, ratio, total)
	return true
}
