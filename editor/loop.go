...
func applyCtlxPrefix(second uint32) uint32 {
	second = view.DecodeKeyChar(second, true)
...
func editorReadKey(keyOut *uint32) bool {
	for {
		...
		*keyOut = view.ApplyMetaPrefixToKey(k)
		return true
	}
}
...
// drainEvents processes queued events until empty. Returns false to exit the editor.
func drainEvents() bool {
	for {
		select {
		case e := <-event.Chan():
			if !Handle(model.State, e) {
				return false
			}
		default:
			return true
...
func Run(e *Editor) {
	...
	EnsureServices()
	term.Open()
	model.ClipboardReady = clipboard.Init() == nil
	view.DisplayInit()
