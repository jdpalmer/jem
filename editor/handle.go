...
type Listener interface {
	Handle(s *model.AppState, e event.Event) ListenerResult
}
...
func Handle(s *model.AppState, e event.Event) bool {
	if s == nil {
		s = model.State
	}
	if e == nil {
...
func handlePasteEvent(ev event.PasteEvent) bool {
	...
	if model.State.ActiveMinibuffer != nil {
		ok = model.MinibufferInsertPaste(ev.Data)
	} else {
		ok = model.InsertPaste(model.State.CurrentWindow, ev.Data)
	}
	if ok {
		model.MarkPasteDirty()
		view.NotePasteApplied()
	}
	return true
}

func handleQuitEvent(s *model.AppState, ev event.QuitEvent) bool {
	if !ev.Force && anyUnsavedBuffers() {
		...
		view.MBWrite("%s (y/n)", "Quit with unsaved buffers?")
		return true
	}
	return false
}
